...
| 2026-08-07 | Does editing a pull-request body re-run the gate? | **No**, not on `pull_request: [synchronize, labeled, …]` alone — `edited` and `issue_comment` are separate triggers. A body fixed without a push leaves the check red until it is re-run. `gh run rerun --failed` is enough: the gate reads the body through the API at run time, so it sees the correction. Adding `issue_comment` is not free — that event runs the workflow from the **default branch**, so a file-route entry would be read off `main` rather than off the branch under test. | PR #16 |
